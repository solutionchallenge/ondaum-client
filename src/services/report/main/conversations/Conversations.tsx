...
function Conversations() {
    ...
    const fetchChats = async () => {
        const response = await getChats({
          datetime_gte:dayjs().subtract(Number(searchFilterBottomSheet.filter.date), 'day').toISOString(),
    ...
    useEffect(()=>{
        fetchChats();
    },[searchFilterBottomSheet.filter.date, searchFilterBottomSheet.filter.emotion, searchBottomSheet.keyword]);
        ...
        <section className="py-2 px-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              onClick={() => {
                setSearchFilterBottomSheet({...searchFilterBottomSheet, isVisible: true});
              }}
            >
              {searchFilterBottomSheet.filter.date} days ago · {searchFilterBottomSheet.filter.emotion.join(',')} <ArrowDownIcon />
            </button>
          </div>
          <span className="block text-main">{dayjs().subtract(Number(searchFilterBottomSheet.filter.date), 'day').format('YYYY.MM.DD')} - {dayjs().format('YYYY.MM.DD')}</span>
        </section>
