  ...
  const data = [
    {
      ...
      joy: report?.emotion_counts?.find((item) => item.emotion === "joy")
        ?.count,
      sadness: report?.emotion_counts?.find(
        (item) => item.emotion === "sadness"
      )?.count,
      anger: report?.emotion_counts?.find((item) => item.emotion === "anger")
        ?.count,
      surprise: report?.emotion_counts?.find(
        (item) => item.emotion === "surprise"
      )?.count,
      fear: report?.emotion_counts?.find((item) => item.emotion === "fear")
        ?.count,
      disgust: report?.emotion_counts?.find(
        (item) => item.emotion === "disgust"
      )?.count,
    },
  ];
        ...
        <div className="relative w-24 h-24">
          <>
            <RadialBarChart
              ...
              data={[
                {
                  negative: report?.average_negative_score
