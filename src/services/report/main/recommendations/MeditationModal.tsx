...
function MeditationModal({item, updateItem}: MeditationModalProps){
  ...
  return (
    <Modal isOpen={item.isVisible} onClose={()=>{
      updateItem({...item, isVisible: false});
    }}>
      <>
          <video id="video" className="w-full h-full rounded-lg" src={item.videoSrc}/>  
          <h2 className="text-2xl font-bold text-font-color mb-1 mt-3">{item.title}</h2>
          ...
          <button className="flex items-center justify-center w-full py-3 rounded-lg bg-orange-400 text-white font-semibold" onClick={startPlay}>
